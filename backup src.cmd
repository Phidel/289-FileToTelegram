...
set arc=%ProjectNo%_sources_%datestr%_%exename%_%vers%.rar

"C:\Program Files\WinRAR\WinRAR.exe" a "%arc%" *.* C:\Comp\DeaTools\*.* -r -ed -v1G -m1 -msrar;zip;jpg;jpeg;png;mp3 -x*.exe -x"*\for-client\*.*" -x"\data\" -x"*\__history\*" -x"*\__recovery\*" -xlog*.txt -x"Dcu"

if %errorlevel%==0 (
   echo %TIME% Архивирование успешно завершено
   rem call D:\Dropbox\Utils\mf.cmd %arc%
